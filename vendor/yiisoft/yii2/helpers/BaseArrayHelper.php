...
use yii\base\InvalidParamException;
	...
	/*const DATE_FORMAT1 = 'php:d/m/Y';
	const DATETIME_FORMAT1 = 'php:Y-m-d H:i:s';
    const TIME_FORMAT1 = 'php:H:i:s';*/
/**
...
class BaseArrayHelper
{
	...
	public static function invconverter($array,$variable)
	{
		foreach($array as $arr=>$imp)
		{
	...
	public static function checkUrl($place)
	{
...
@$po=explode("&",$ex[1]);
$ex[1]=$po[0];
$currentUrl=str_replace("%2F","/",$ex[1]);
...
if(!strstr($currentUrl,"/"))
{
	...
	if(strstr($currentUrl,"index") and $currentUrl!="site/index")
	{
		//$currentUrl=trim($currentUrl,"index");
	}
	  $session = Yii::$app->session;
	$check=$session['auth'];
	if(is_object($check))
	{
	$urlList=($check->url['url_name']);


if(in_array($currentUrl,$urlList))
{
	return true;
}
else
{	
	return false;
		...
		public static function warnalert($text,$val)
		{
			if($val==1)
			{
		return $text1='<div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                    <h4><i class="icon fa fa-check"></i>'.$text.'</h4>
                  </div>';
		}
		elseif($val==2){
			
		return $text1='<div class="alert alert-warning alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
		...
		return $text1='<div class="alert alert-danger alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
        ...
        return  Yii::$app->formatter->asDate($dateStr, $fmt);
			...
			public static function convertback($dateStr, $type='date', $format = null) {
			
			
        if ($type === 'datetime') {
              $fmt = ($format == null) ? self::DATETIME_FORMAT1 : $format;
        }
        elseif ($type === 'time') {
              $fmt = ($format == null) ? self::TIME_FORMAT1 : $format;
        }
        else {
              $fmt = ($format == null) ? self::DATE_FORMAT1 : $format;
        }
        return Yii::$app->formatter->asDate($dateStr, $fmt);
    }
		
		
		public static function datevolume($dateConvert,$volume,$condition)
		{
			if($condition=="create")
			{
			unset($dateConvert['_csrf']);
		    foreach($dateConvert as $mainhead=>$convert)
		{
			foreach($convert as $var=>$val)
			{
				if(strstr($var,"date"))
				{
					
					
					$dateConvert[$mainhead][$var]=self::convert($val,$volume);
				}
			}
		}
			return $dateConvert;
				...
				foreach($dateConvert as $var=>$val)
                {
	          if(strstr($var,"date"))
	         {
	         $back= self::convertback($val);	
	       
			$dateConvert->$var=$back;
	         }
             }
             return $dateConvert;
			}
